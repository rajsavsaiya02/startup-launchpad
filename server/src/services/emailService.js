...
const getTransporter = async () => {
    ...
    const configPort = parseInt(settings.smtp_port) || process.env.SMTP_PORT || 587;
    const isSecure = settings.smtp_secure === true || String(settings.smtp_secure) === 'true';

    // Log AFTER variables are ready or use source variables
    console.log(`[DEBUG] Email Service Config: Host=${settings.smtp_host} Port=${configPort} Secure=${isSecure}`);
...
const sendVerificationEmail = async (to, otp) => {
    try {
        ...
        const fromEmail = settings.system_email_address || 'no-reply@launchpad.com';
...
const sendEmail = async ({ to, subject, html, text }) => {
    try {
        ...
        if ((!fromAddress || fromAddress.includes('no-reply@launchpad.com')) && settings.smtp_user) {
