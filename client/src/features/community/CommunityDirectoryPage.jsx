        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-slate-600 dark:text-slate-400 max-w-2xl mx-auto mb-12 font-medium leading-relaxed">
              Meet the founders, creators, and visionaries building the next
              generation
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
        <div className="flex justify-between items-center mb-10 px-2">
          ...
          <div className="hidden sm:flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-widest">
            Sorted By{" "}
            <span className="text-primary italic">Recently Joined</span>
          </div>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className="h-80 bg-white dark:bg-surface-dark rounded-2xl animate-pulse border border-slate-100 dark:border-white/5"
              />
            ))}
          </div>
        ) : data.length === 0 ? (
          ...
          >
            ...
            <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-10 leading-relaxed font-medium">
              We couldn't find any profiles matching your search. Try expanding
              your criteria or clear the filters.
            </p>
            <Button
              onClick={() => {
                setSearch("");
...
function ProfileCard({ profile }) {
  ...
  const avatarUrl = profile.avatar_url
    ? profile.avatar_url.startsWith("http") ||
      profile.avatar_url.startsWith("/public-assets") ||
      profile.avatar_url.startsWith("/public")
      ? profile.avatar_url
      : `${SERVER_URL}${profile.avatar_url}`
    : null;

  const defaultAvatar = isOrg
    ? `https://ui-avatars.com/api/?name=${encodeURIComponent(profile.name)}&background=0284c7&color=fff&size=200&bold=true`
  ...
  const getRoleDisplay = () => {
    if (isOrg)
      return {
        label: "Startup",
        color: "text-blue-600",
        bg: "bg-linear-to-br from-blue-500/20 to-indigo-500/10 dark:from-blue-600/30 dark:to-indigo-600/10",
    ...
    switch (profile.role) {
      case "founder":
        return {
          ...
          bg: "bg-linear-to-br from-amber-400/20 to-orange-400/10 dark:from-amber-500/30 dark:to-orange-500/10",
      ...
      case "freelancer":
        return {
          ...
          bg: "bg-linear-to-br from-emerald-400/20 to-teal-400/10 dark:from-emerald-500/30 dark:to-teal-500/10",
      ...
      case "student":
        return {
          label: "Student",
          color: "text-cyan-600",
          bg: "bg-linear-to-br from-cyan-400/20 to-blue-400/10 dark:from-cyan-500/30 dark:to-blue-500/10",
          icon: <Users className="h-3 w-3" />,
        };
      case "normal_user":
      default:
        return {
          label: "Talent",
          color: "text-rose-600",
          bg: "bg-linear-to-br from-rose-400/20 to-orange-400/10 dark:from-rose-500/30 dark:to-orange-500/10",
          icon: <Users className="h-3 w-3" />,
        };
    }
  };
  ...
  return (
    ...
    >
      <Link to={linkTo} className="block h-full outline-none">
        <div className="relative h-full bg-white dark:bg-surface-dark border border-slate-100 dark:border-white/5 rounded-4xl overflow-hidden hover:shadow-2xl hover:shadow-primary/10 transition-all duration-500 flex flex-col group-hover:-translate-y-2 group-hover:border-primary/20">
          ...
          <div className="px-8 pb-10 flex-1 flex flex-col text-center">
            ...
            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mb-6 line-clamp-2 min-h-[40px]">
              {profile.headline ||
                profile.department ||
                (isOrg ? "Emerging Startup" : "Network Member")}
            </p>
