...
import { Layout, FileText, Phone, Info, Plus, Search, Filter, MoreVertical, Edit, Globe, Trash2, Eye, ArrowRight } from 'lucide-react';
...
export function PublicPageManager() {
  ...
  // Filter
  const finalDisplayPages = mergedPages.filter(p => 
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-border-light dark:border-border-dark pb-6">
        <div>
          <h1 className="text-3xl font-bold text-text-primary dark:text-white tracking-tight">Public Page Manager</h1>
          <p className="text-text-secondary dark:text-gray-400 mt-2 text-lg">Manage all your public-facing content and navigation.</p>
        </div>
        <div className="flex gap-3">
             <Button variant="outline" onClick={() => navigate('/admin/communication/cms/navigation')}>
                <Filter className="h-4 w-4 mr-2" />
                Manage Navigation
             </Button>
            <Button size="lg" variant="primary" onClick={() => setIsCreateOpen(true)} className="shadow-lg shadow-primary/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6">
        {loading ? (
            ...
            finalDisplayPages.map((page) => {
                ...
                return (
                    <Card key={page.slug} className="group relative overflow-hidden flex flex-col h-full hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border-border-light dark:border-border-dark">
                        
                        {/* Preview / Top Area */}
                        <div className={`h-48 w-full ${page.color} bg-opacity-10 dark:bg-opacity-20 flex items-center justify-center relative overflow-hidden group-hover:bg-opacity-15 transition-all duration-300`}>
                            {page.exists ? (
                                ...
                                <>
                                    <div className={`absolute -right-6 -top-6 w-24 h-24 rounded-full ${page.color} opacity-20 group-hover:scale-150 transition-transform duration-500`} />
                                    <div className={`p-4 rounded-xl ${page.color} bg-opacity-20 text-${page.color.replace('bg-', '')}-600 dark:text-white mb-2 z-10`}>
                             ...
                             <div className="absolute top-3 right-3 z-20">
                                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border shadow-sm ${
                                    page.status === 'published' ? 'bg-white text-green-700 border-green-200 dark:bg-gray-900 dark:text-green-400 dark:border-green-800' : 
                                    page.status === 'draft' ? 'bg-white text-amber-700 border-amber-200 dark:bg-gray-900 dark:text-amber-400 dark:border-amber-800' : 
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                            <div className="flex justify-between items-start mb-2">
                                <h3 className="text-lg font-bold text-text-primary dark:text-white group-hover:text-primary transition-colors">{page.title}</h3>
                            </div>
                            ...
                            <div className="flex items-center text-xs text-text-tertiary mb-4 space-x-2">
                                <span className="font-mono bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded">/{page.slug}</span>
                                {page.updated_at && <span>• {format(new Date(page.updated_at), 'MMM d, yyyy')}</span>}
                            </div>
                                ...
                                {page.exists ? (
                                    <>
                                        <Button 
                                            ...
                                            onClick={() => navigate(`/admin/communication/cms/pages/${page.slug}`)}
                                        >
                                    ...
                                    <Button 
                                        ...
                                        size="sm"
                                        className="w-full"
                                        onClick={() => handleCreateSystemPage(page.slug, page.title)}
                                    >
