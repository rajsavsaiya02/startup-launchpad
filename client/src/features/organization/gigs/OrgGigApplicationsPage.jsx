...
export function OrgGigApplicationsPage() {
  const { id } = useParams();
        ...
        <div className="flex flex-wrap justify-between items-end gap-4">
          <div>
            <h1 className="text-3xl font-bold text-text-primary dark:text-white mb-1">
              Applications for UI/UX Designer Role
            </h1>
            <p className="text-text-secondary dark:text-gray-400 text-sm">
              17 candidates applied
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {APPLICANTS.map((app) => (
            <Card
              key={app.id}
              className="p-5 hover:shadow-md transition-shadow bg-white dark:bg-surface-dark border-border-light dark:border-border-dark"
            >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm text-text-secondary dark:text-gray-300 my-4">
                    <span className="flex items-center gap-1">
                      <BriefcaseIcon className="h-3 w-3" /> {app.exp}
                    </span>
                    <span className="text-border-light dark:text-border-dark">
                      |
                    </span>
                    <span className="flex items-center gap-1">
                      <MapPin className="h-3 w-3" /> {app.loc}
                    </span>
                    <span className="text-border-light dark:text-border-dark">
                      |
                    </span>
                  ...
                  <div className="flex gap-3 mt-auto">
                    ...
                    <Button
                      variant="ghost"
                      className="flex-1 text-error hover:text-error hover:bg-error/10"
                    >
                      Reject
                    </Button>
                  </div>
