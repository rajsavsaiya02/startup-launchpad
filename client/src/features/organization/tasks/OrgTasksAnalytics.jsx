...
const KPICard = ({ title, value, icon: Icon, colorClass, tooltip, subtext }) => (
  <Card className="relative overflow-hidden p-5 flex flex-col justify-between border border-border-light dark:border-border-dark bg-white/60 dark:bg-surface-dark/60 backdrop-blur-xl shadow-sm hover:shadow-lg transition-all duration-300 group rounded-3xl h-full">
    ...
    <div
      className={cn(
        "absolute -right-4 -top-4 w-16 h-16 rounded-full blur-xl opacity-20 group-hover:opacity-40 transition-opacity duration-500",
    ...
    <div className="flex items-center justify-between mb-4 relative z-10">
      <div
        className={cn(
          "p-2.5 rounded-2xl bg-gray-50 dark:bg-gray-800/50 ring-1 ring-inset ring-gray-900/5 dark:ring-white/5",
          colorClass,
        )}
      >
...
export function OrgTasksAnalytics({ tasks }) {
  ...
  const metrics = useMemo(() => {
    ...
    // Task Activity Distributions (Priority breakdown)
      ...
      logs.forEach((log) => {
        ...
        try {
          ...
          if (seconds > 0) {
            ...
            if (seconds >= 3600) deepWorkSeconds += seconds;

            const hour = getHours(start);
            const day = getDay(start);

            heatmapMatrix[day][hour] += Math.round(seconds / 60);
    ...
    const peakInsight =
      maxMins > 0
        ? `Your team performs best on ${daysMap[peakDay]}s around ${peakHour % 12 || 12} ${peakHour >= 12 ? "PM" : "AM"}.`
        : "Start tracking time to discover your team's peak performance hours.";
    ...
    return {
      ...
      deepWorkRatio:
        totalSecondsTracked > 0
          ? Math.round((deepWorkSeconds / totalSecondsTracked) * 100)
          : 0,
      avgLeadTimeSeconds,
      hourlyDistribution,
      dailyDistribution,
  ...
  const getHeatmapOption = useMemo(() => {
    ...
    return {
      ...
      visualMap: {
        min: 0,
        max: Math.max(60, ...metrics.heatmapMatrix.flat()),
        calculable: true,
        orient: "horizontal",
        left: "center",
        bottom: "0%",
        inRange: {
          color: ["#f3f4f6", "#c7d2fe", "#818cf8", "#4f46e5", "#312e81"],
        },
  ...
  const timeEngagementOption = useMemo(() => {
    ...
    return {
      ...
      yAxis: {
        type: "value",
        splitLine: {
          lineStyle: { type: "dashed", color: "rgba(107,114,128,0.1)" },
        },
  ...
  return (
    <div className="flex-1 overflow-auto p-4 md:p-6 lg:p-8 space-y-8 bg-transparent w-full pb-20">
      ...
      >
        ...
        <div>
          <h4 className="font-bold text-sm tracking-wide">Team Insight</h4>
      ...
      >
        <KPICard
          ...
          tooltip="Percentage of total org tasks marked as done."
        ...
        <KPICard
          ...
          tooltip="Total time actively tracked using the timer across all org tasks."
          subtext="Collectively tracked"
        />
        <KPICard
          ...
          tooltip="Percentage of tracked time spent in sessions ≥ 1 hour."
        ...
        <KPICard
          ...
          tooltip="Average time from org task creation to completion."
            ...
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <p className="text-sm text-text-tertiary mt-1 font-medium">
                  Discover your organization's optimal focus windows.
                </p>
              </div>
            </div>
        ...
        >
          <Card className="p-6 border border-border-light dark:border-border-dark bg-white/60 dark:bg-surface-dark/60 backdrop-blur-xl rounded-3xl shadow-sm h-full flex flex-col">
            <div className="mb-6">
              ...
              <p className="text-sm text-text-tertiary mt-1 font-medium">
                Current distribution across the organization.
              </p>
            </div>
          ...
          <Card className="p-6 border border-border-light dark:border-border-dark bg-white/60 dark:bg-surface-dark/60 backdrop-blur-xl rounded-3xl shadow-sm flex flex-col h-[300px]">
            <div className="flex items-center justify-between mb-4 z-10 relative">
              <div>
                ...
                <p className="text-[10px] text-text-tertiary mt-0.5 font-bold uppercase tracking-wider">
                  Tracking Trends
                </p>
              </div>
