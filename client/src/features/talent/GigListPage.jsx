...
import { Search, Filter, Briefcase, DollarSign, Clock, MapPin, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-border-light dark:border-border-dark pb-6">
        <div>
          <h1 className="text-3xl font-bold text-text-primary dark:text-white">Find Gigs</h1>
          <p className="text-text-secondary dark:text-gray-400 mt-1">Browse and apply for short-term contracts and projects.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {GIGS.map((gig) => (
          <Card key={gig.id} className="p-6 hover:shadow-md transition-all flex flex-col h-full bg-white dark:bg-surface-dark border-border-light dark:border-border-dark">
            <div className="flex justify-between items-start mb-4">
              <div className="flex gap-3">
                <Avatar src={gig.img} size="md" />
                <div>
                  <h3 className="font-bold text-text-primary dark:text-white line-clamp-1">{gig.title}</h3>
                  <p className="text-sm text-text-secondary dark:text-gray-400">{gig.org}</p>
                </div>
              </div>
              <Badge variant="neutral" className="shrink-0">{gig.type}</Badge>
            </div>

            <div className="flex flex-wrap gap-2 mb-6">
              {gig.skills.map(s => <Badge key={s} variant="neutral" className="bg-gray-100 dark:bg-gray-800 border-transparent">{s}</Badge>)}
            </div>
            ...
            <div className="mt-auto space-y-4">
              <div className="flex justify-between text-sm text-text-secondary dark:text-gray-400">
                ...
                <span className="flex items-center gap-1"><Clock className="h-4 w-4" /> {gig.posted}</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Link to={`/gigs/${gig.id}/applications`} className="w-full">
