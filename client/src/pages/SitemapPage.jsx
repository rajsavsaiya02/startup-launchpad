    ...
    async function loadSitemap() {
      try {
        ...
        setPages(
          data.filter(
            (p) =>
              !["home", "homepage", "home-page"].includes(p.slug.toLowerCase()),
          ),
        );
      } catch (err) {
              ...
              <ul className="space-y-3">
                {staticRoutes.map((route, i) => (
                  <li key={i}>
                    <Link
                      to={route.path}
                      className="flex items-center group p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors"
                    >
            ...
            <div>
              ...
              {pages.length === 0 ? (
                <p className="text-gray-500 italic">
                  No additional content pages found.
                </p>
              ) : (
                <ul className="space-y-3">
                  {pages.map((page) => (
                    <li key={page.slug}>
                      <Link
                        to={`/${page.slug}`}
                        className="flex items-center justify-between group p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors"
                      >
