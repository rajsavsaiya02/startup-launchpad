...
import { Menu, X, Rocket, ChevronDown, LayoutDashboard, Wallet, Users, BrainCircuit, BookOpen, HelpCircle, FileText, Info, Mail, Shield } from 'lucide-react';
...
const IconRenderer = ({ icon, className }) => {
  ...
  const IconMap = {
    LayoutDashboard, Wallet, Users, BrainCircuit, BookOpen, HelpCircle, FileText, Info, Mail, Shield,
...
export function PublicHeader() {
  ...
  const activeMenu = (settings?.navigation_menu && Array.isArray(settings.navigation_menu) && settings.navigation_menu.length > 0) 
      ...
      <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
                ...
                {menuItem.items && (
                  <div className={cn(
                    "absolute top-full left-1/2 -translate-x-1/2 pt-2 transition-all duration-200 ease-in-out opacity-0 translate-y-2 pointer-events-none z-40",
                     // Hover Logic
                     hoveredMenu === menuItem.label && "opacity-100 translate-y-0 pointer-events-auto"
                  )}>
                    {menuItem.type === 'mega' ? (
                      <div className="w-[600px] p-6 bg-white dark:bg-[#1e1a29] rounded-xl shadow-2xl border border-border-light dark:border-border-dark grid grid-cols-2 gap-4">
                        {menuItem.items.map((subItem) => (
                           ...
                           >
                              <div className={cn("h-10 w-10 rounded-lg flex items-center justify-center shrink-0 transition-colors", subItem.color)}>
                                <IconRenderer icon={subItem.icon} className="h-5 w-5" />
                              </div>
                              <div>
                      ...
                      <div className="w-56 p-2 bg-white dark:bg-[#1e1a29] rounded-xl shadow-xl border border-border-light dark:border-border-dark flex flex-col gap-1">
                        {menuItem.items.map((subItem) => (
                           ...
                           >
                              <IconRenderer icon={subItem.icon} className="h-4 w-4" />
                              {subItem.name}
                           </Link>
                        ))}
                      </div>
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            ...
            <>
              <Link to="/auth/login">
                <Button variant="ghost" className="font-semibold text-text-secondary dark:text-gray-300 hover:text-text-primary">
                  Log In
                </Button>
              </Link>
              <Link to="/auth/signup">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-surface-light dark:bg-background-dark pt-20 px-4 pb-8 overflow-y-auto animate-in slide-in-from-right-10 duration-200 md:hidden">
          <nav className="flex flex-col gap-1">
            {activeMenu.map((menuItem) => (
               <div key={menuItem.label} className="border-b border-border-light dark:border-border-dark/50 last:border-0">
                 {menuItem.type === 'link' ? (
                   <NavLink
                      to={menuItem.path || menuItem.url}
                   ...
                   <div>
                     ...
                     >
                       {menuItem.label}
                       <ChevronDown className={cn("h-5 w-5 transition-transform", mobileExpanded[menuItem.label] ? "rotate-180" : "")} />
                     </button>
                     ...
                     {mobileExpanded[menuItem.label] && (
                        <div className="pb-4 pl-2 space-y-2">
                           {menuItem.items.map((subItem) => (
                              ...
                              >
                                {menuItem.type === 'mega' && (
                                  <div className={cn("h-8 w-8 rounded-md flex items-center justify-center shrink-0", subItem.color?.split(' ')[1] || 'bg-gray-100')}>
                                     <IconRenderer icon={subItem.icon} className="h-4 w-4" />
                                  </div>
                                )}
          ...
          <div className="mt-8 space-y-3">
             {user ? (
                ...
                <>
                  <Link to="/auth/login"><Button size="lg" variant="outline" className="w-full">Log In</Button></Link>
                  <Link to="/auth/signup"><Button size="lg" className="w-full">Get Started</Button></Link>
                </>
             )}
          </div>
        </div>
