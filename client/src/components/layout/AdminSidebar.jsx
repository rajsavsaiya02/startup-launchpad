...
const MODULE_NAV_CONFIG = {
  ...
  communication: [
    ...
    {
      name: "Blog Posts",
      path: "/admin/communication/cms/blogs",
      icon: FileText,
    }, // Merged
    {
      name: "Public Page Manager",
      path: "/admin/communication/cms/homepage",
      icon: Globe,
    }, // Merged
    {
      name: "Testimonials",
            ...
            {isCollapsed && (
              <div className="absolute left-[calc(100%+10px)] top-1/2 -translate-y-1/2 hidden group-hover:block z-50">
                <div className="bg-gray-900 text-white text-xs font-medium px-2 py-1.5 rounded-md shadow-lg whitespace-nowrap relative animate-in fade-in zoom-in-95 duration-200">
                  {item.name}
                  {/* Triangle pointer */}
                  <div className="absolute top-1/2 -left-1 -translate-y-1/2 w-0 h-0 border-y-4 border-y-transparent border-r-4 border-r-gray-900"></div>
                </div>
              </div>
            )}
          </NavLink>
